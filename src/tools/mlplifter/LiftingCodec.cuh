...
namespace Flair
{
	...
	{
	public:
		using InputSample = Tensor1D<35>;
		using OutputSample = Tensor1D<25>;
	...
	private:
		...
		__host__ std::tuple<InputSample, float, float, float> GenerateInputSample(const float x, const float y, UniformDistribution* uniform) const;
		__host__ OutputSample GenerateTargetSample(const float x, const float y, const std::tuple<InputSample, float, float, float>& inputSample) const;
